...
class MoveGrasp(object):
    ...
    def collectTactileDataRountine(self, data):

        count = 10
        while count > 0:
            if count % 2 == 0:
                print('now hold this one')
            else:
                print('release this one')
            self.filename_publish(data.data + '_' + str(count))
            self.robotiq_move(255, 70, 0)
            rospy.sleep(2)
            self.robotiq_move(0, 0, 0)
            rospy.sleep(4)
            print(count)
            ...
            self.filename_publish("stop")
    def collectTactileDataRountine2(self, data):
        """This one is used for stiff test to test soft reset
    ...
    def collectTactileDataRountine3(self, data):
        """This one is used for force analysis
        """
        count = 20
        while count > 0:
            self.filename_publish(data.data + '_' + str(count))
            self.robotiq_move(255, 0, 0) # minimum force and minimum speed
            rospy.sleep(4)
            self.robotiq_move(0, 0, 0)
            rospy.sleep(10)
            count-=1
    ...
    def cmdCallback(self, data):
        ...
        #    self.data_collection_routine(data)
            ...
            self.collectTactileDataRountine3(data)
