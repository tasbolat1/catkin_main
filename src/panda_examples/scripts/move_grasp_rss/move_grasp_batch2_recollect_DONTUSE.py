...
class MoveGrasp(object):
    ...
    def move_to_z(self,z):
        ...
        if height >= 0.160 and height <= 0.7: # 0.247
            self.go_to_cartesian_pose([0.6,0,height],[0.5**0.5, -(0.5**0.5),0,0])
        else:
            print("DANGEROUS")
    ...
    def trajectory_saver(self, z):
        ...
        if not self.is_moveit_inited:
            self.move_it_init()
        self.go_to_joint_pose([0,0.579293,0,-2.01618,0,2.59344,1.5707963])
        self.cartesian_plan_save_execute(0.5, [0.6,0,height/1000.0],[0.5**0.5, -(0.5**0.5),0,0],'v2_fakehome_obj_'+str(height)+'.yaml')
        self.cartesian_plan_save_execute(0.5, [0.6,0,height/1000.0+0.1],[0.5**0.5, -(0.5**0.5),0,0],'v2_obj_preproph_'+str(height)+'.yaml')
        self.cartesian_plan_save_execute(0.3, [0.6,0,height/1000.0],[0.5**0.5, -(0.5**0.5),0,0],'v2_preproph_obj_'+str(height)+'.yaml')
        self.cartesian_plan_save_execute(0.3, [0.6,0,height/1000.0+self.lift_height],[0.5**0.5, -(0.5**0.5),0,0],'v2_obj_5cm_'+str(height)+'.yaml')
        self.go_to_joint_pose([0,0.579293,0,-2.01618,0,2.59344,1.5707963])
        self.routine_unlock()
    ...
    def move_to_obj_height(self, data):
        ...
        if not self.pose_check([0.6,0,height/1000.0],[0.5**0.5, -(0.5**0.5),0,0]):
    ...
    def data_collection_routine(self, data):
        ...
        if not self.pose_check([0.6,0,height/1000.0+0.1],[0.5**0.5, -(0.5**0.5),0,0]):
            self.something_wrong()
            return
        ...
        if cur_time + stage1_duration - time.time() < 0 or not self.pose_check([0.6,0,height/1000.0],[0.5**0.5, -(0.5**0.5),0,0]):
            self.something_wrong()
            return
        ...
        if cur_time + stage2_duration - time.time() < 0 or not self.pose_check([0.6,0,height/1000.0+self.lift_height],[0.5**0.5, -(0.5**0.5),0,0]):
            self.something_wrong()
            return
