...
class MoveGrasp(object):
    def __init__(self):
        ...
        self.unshrink = {
            'a':'004_sugar_box',
            'b':'071_nine_hole_peg_test',
            'c':'003_cracker_box',
            'd':'006_mustard_bottle',
            'e':'010_potted_meat_can',
            'f':'065-a_cups',
            'g':'065-b_cups',
            'h':'065-c_cups',
            'i':'065-d_cups',
            ...
            's':'101_rope',
            't':'046_plastic_bolt',
            'u':'072-f_toy_airplane',
            'v':'072-d_toy_airplane',
            'w':'072-j_toy_airplane',
            'x':'072-i_toy_airplane',
            'y':'041_small_marker',
            'z':'040_large_marker',
            'aa':'102-a_duck',
            'bb':'102-b_duck',
            ...
            'ee2':'103-b_evian',
            'ee3':'103-c_evian',
            'ff1':'107-a_pepsi_bottle',
            'ff2':'107-b_pepsi_bottle',
        }
    ...
    def unshrink_classname(self, shrunk):
        return self.unshrink[shrunk]

    def move_to_obj_height(self, data):
        try:
            height = int(self.height_dict[self.unshrink_classname(data.data[5:])])
            self.play_sound(0)
        except KeyError:
            self.play_sound(2)
    ...
    def data_collection_routine(self, data):
        if os.path.isfile(self.unshrink_classname(data.data[0:-3])+data.data[-3:]+'.startend'):
            self.play_sound(2)
            print('FILE EXISTS')
            return
        len_a = len(os.listdir('/home/crslab/aces_recordings'))
        len_f = len(os.listdir('/home/crslab/franka_states'))
        len_r = len(os.listdir('/home/crslab/rbtq_states'))
        len_t = len(os.listdir('/home/crslab/traj_start_ends'))
        ...
        try:
            height = int(self.height_dict[self.unshrink_classname(data.data[0:-3])])
            self.play_sound(0)
        except KeyError:
            self.play_sound(2)
            print("Filename not recognized, most likely typo.")
        ...
        self.f = open(self.unshrink_classname(data.data[0:-3])+data.data[-3:]+".startend", "w")
        ...
        self.load_plan_execute('/home/crslab/franka_traj/v2_obj_preproph_'+str(height)+'.yaml')
        ...
        self.filename_publish(self.unshrink_classname(data.data[0:-3])+data.data[-3:])
        self.traj_timestamps_holder = list() #wipe list
        self.timestamp_append()
        rospy.sleep(0.5)
        cur_time = time.time()
        self.load_plan_execute('/home/crslab/franka_traj/v2_preproph_obj_'+str(height)+'.yaml')
        ...
        cur_time = time.time()
        self.load_plan_execute('/home/crslab/franka_traj/v2_obj_5cm_'+str(height)+'.yaml')
