...
class MoveGrasp(object):
    ...
    def load_plan_execute(self, plan_file):
        ...
        self.timestamp_append() #start executing plan
        group.execute(loaded_plan, wait=True)
        group.stop()
        self.timestamp_append() #stop executing plan
        group.clear_pose_targets()
        return True
    ...
    def move_to_z(self,z):
        ...
        if height >= 0.260 and height <= 0.7: # 0.247
            self.go_to_cartesian_pose([0.4,0,height],[0.5**0.5, -(0.5**0.5),0,0])
        else:
            print("DANGEROUS")
    ...
    def trajectory_saver(self, z):
        height = int(z.data[1:]) #in mm
        self.routine_lock()
        if not self.is_moveit_inited:
            self.move_it_init()
        self.go_to_joint_pose([0,-0.221883,0,-2.70201,0,2.4795,1.5707963])
        self.cartesian_plan_save_execute(0.5, [0.4,0,height/1000.0],[0.5**0.5, -(0.5**0.5),0,0],'v2_fakehome_obj_'+str(height)+'.yaml')
        self.cartesian_plan_save_execute(0.5, [0.4,0,height/1000.0+0.1],[0.5**0.5, -(0.5**0.5),0,0],'v2_obj_preproph_'+str(height)+'.yaml')
        self.cartesian_plan_save_execute(0.3, [0.4,0,height/1000.0],[0.5**0.5, -(0.5**0.5),0,0],'v2_preproph_obj_'+str(height)+'.yaml')
        self.cartesian_plan_save_execute(0.3, [0.4,0,height/1000.0+self.lift_height],[0.5**0.5, -(0.5**0.5),0,0],'v2_obj_5cm_'+str(height)+'.yaml')
        self.go_to_joint_pose([0,-0.221883,0,-2.70201,0,2.4795,1.5707963])
        self.routine_unlock()
    ...
    def gripper_map(self, widthmm):
        #open = 0, close = 255
        gripper_value = 255-((255/0.140)* widthmm) # assume linear mapping
        gripper_value = int(math.ceil(gripper_value))
        return gripper_value
        
    def data_collection_routine(self, data):
        if os.path.isfile(data.data+'.startend'):
            self.play_sound(2)
            print('FILE EXISTS')
            return
        len_a = len(os.listdir('/home/crslab/hh_ait/160320/neutouch_data'))
        len_f = len(os.listdir('/home/crslab/hh_ait/160320/franka_states'))
        len_r = len(os.listdir('/home/crslab/hh_ait/160320/rbtq_states'))
        len_t = len(os.listdir('/home/crslab/hh_ait/160320/traj_start_ends'))
        if not len_f * 2 == len_a or not len_r * 2 == len_a or not len_t * 2 == len_a:
            self.play_sound(2)
            print('COCKUP',len_a,len_f,len_r,len_t)
            return

        try:
            height = int(self.height_dict[data.data[0:-3]])  ####object name corresponding to objects_heights.csv
            self.play_sound(0)
        except KeyError:
            self.play_sound(2)
            print("Filename not recognized, most likely typo.")
            return
        #height = 455 #for debug
        self.routine_lock()
        if not self.is_moveit_inited:
            self.move_it_init()
        stage1_duration = 2.0 #Max duration of trajectory estimated to be 7.5, for z=0.26
        stage2_duration = 2.0  #Max duration of trajectory estimated to be 1.15, for z=0.26
        self.f = open(data.data+".startend", "w")
        #self.joint_plan_save_execute([0, -0.496787628232387, 0, -2.1629668687145287, 0, 1.664665088957994, 1.571265406104273],'first_joint.yaml')


        self.load_plan_execute('/home/crslab/franka_traj/v2_obj_preproph_'+str(height)+'.yaml')  #execute plan to home position
        if not self.pose_check([0.4,0,height/1000.0+0.1],[0.5**0.5, -(0.5**0.5),0,0]):
            self.something_wrong()
            return
        ...
        self.timestamp_append() #start time timestamp1
        rospy.sleep(0.5)
        cur_time = time.time()
        self.load_plan_execute('/home/crslab/franka_traj/v2_preproph_obj_'+str(height)+'.yaml') #execute plan to grasping position timestamp2,3
        print('Preproph to obj:', time.time()-cur_time)
        if cur_time + stage1_duration - time.time() < 0 or not self.pose_check([0.4,0,height/1000.0],[0.5**0.5, -(0.5**0.5),0,0]):
            self.something_wrong()
            return
        rospy.sleep(cur_time + stage1_duration - time.time())

        self.timestamp_append() #start grasping timestamp4
        self.robotiq_close(0,1)
        rospy.sleep(4.0)


        cur_time = time.time()
        self.load_plan_execute('/home/crslab/franka_traj/v2_obj_5cm_'+str(height)+'.yaml') #timestamp5,6
        print('Obj to 5cm above:', time.time()-cur_time)
        #self.go_to_joint_pose([0, -0.49608036558984253, 0, -2.1629678074167, 0, 1.6650361182424758, 1.5705476338428697])
        if cur_time + stage2_duration - time.time() < 0 or not self.pose_check([0.4,0,height/1000.0+self.lift_height],[0.5**0.5, -(0.5**0.5),0,0]):
            self.something_wrong()
            return
        rospy.sleep(cur_time + stage2_duration - time.time())
        rospy.sleep(0.5)


        self.play_sound(0)
        self.filename_publish("stop")
        print("Trajectory duration (estimated to be 9.01): ",time.time()-traj_start)


        if len(self.traj_timestamps_holder) != 6:
