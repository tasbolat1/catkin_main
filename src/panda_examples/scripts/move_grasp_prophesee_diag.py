...
class MoveGrasp(object):
    ...
    def data_collection_routine(self, data):
        ...
        if not self.is_moveit_inited:
            self.move_it_init()
        # MOVE TO SOYMILK POSITION FIRST
        for i in range(1000):
            self.load_plan_execute('/home/crslab/franka_traj/v2_obj_preproph_285.yaml')
            if not self.pose_check([0.4,0,0.285+0.1],[0.5**0.5, -(0.5**0.5),0,0]):
                self.something_wrong()
                return

            self.filename_publish(data.data+'_'+str(i))
            rospy.sleep(0.5)
            self.load_plan_execute('/home/crslab/franka_traj/v2_preproph_obj_285.yaml')
            rospy.sleep(0.5)
