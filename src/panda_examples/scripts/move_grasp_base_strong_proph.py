...
class MoveGrasp(object):
    def __init__(self):
        ...
        self.rosbag_pub = rospy.Publisher('/filename_rosbag', String, queue_size = 1000)
    ...
    def trajectory_saver(self, data):
        ...
        self.cartesian_plan_save_execute(0.5, [0.55,0,0.35],[0.5**0.5, -(0.5**0.5),0,0],'fakehome_preproph.yaml')
        self.cartesian_plan_save_execute(0.3, [0.55,0,0.25],[0.5**0.5, -(0.5**0.5),0,0],'preproph_obj.yaml')
        self.go_to_joint_pose([0,-0.221883,0,-2.70201,0,2.4795,1.5707963])
        self.routine_unlock()
    ...
    def data_collection_routine(self, data):
        ...
        for iter in range(n_iter):
            ...
            self.play_sound(0)
            self.go_to_joint_pose([0,-0.221883,0,-2.70201,0,2.4795,1.5707963])
            self.load_plan_execute('fakehome_preproph.yaml')
            if not self.pose_check([0.55,0,0.35],[0.5**0.5, -(0.5**0.5),0,0]):
                self.something_wrong()
                return
            rospy.sleep(0.5)
            self.filename_publish(data.data+"_"+str(iter))
            # if iter % 15 == 0:
            #     self.rosbag_pub.publish(data.data+"_"+str(iter))
            rospy.sleep(0.5)
            self.load_plan_execute('preproph_obj.yaml')
            if not self.pose_check([0.55,0,0.25],[0.5**0.5, -(0.5**0.5),0,0]):
                self.something_wrong()
                return
            rospy.sleep(0.5)
            self.filename_publish("stop")
            # if iter % 15 == 0:
            #     self.rosbag_pub.publish("stop")
            rospy.sleep(0.5)
            self.play_sound(0)
            #rospy.sleep(2.5)
