...
class MoveGrasp(object):
    ...
    def print_current_pose(self):
        current_pose = self.group.get_current_pose().pose
        pose_goal = geometry_msgs.msg.Pose()
        print('Quaternion of EE: ', pose_goal.orientation)
        print('Translation of EE: ', pose_goal.position)
        print('here')
    ...
    def data_collection_routine(self, data):
        ...
        self.play_sound(0)
        self.filename_publish("Test")

        self.go_to_home(robotiq = False)

        self.go_to_cartesian_pose([0.55,0,0.415],[0.5**0.5, -(0.5**0.5),0,0])
    ...
    def zh(self):
        ...
        # #q = [ 0.9756423, 0.0167681, -0.2186765, -0.0046348 ]
        # #t = [0.3776718997905275, 0.024374076356996698, 0.2521770859993799]
        # #self.go_to_cartesian_pose(t, q)

        # target_goal = [0.0034665750620023403, -0.1766274640345483, 0.019519778476995323, -2.5084076521020187, 0.005414730209443304, 1.5892380732911158, 0.7857512227454201]
        # self.go_to_joint_pose(target_goal)

        # self.print_current_pose()
        # self.go_to_home(robotiq = False)
        #self.move_gripper(0.08, 0.04, True)
        #self.move_gripper(0.01, 0.01, True)
        #self.go_to_home(robotiq = False)
        #self.home_routine()
        self.routine_unlock()
