...
class MoveGrasp(object):
    ...
    def go_to_home(self, robotiq = True):
        ...
        joint_goal[4] = 0
        joint_goal[5] = pi/2
        joint_goal[6] = pi/4 + robotiq * pi/4 #+ robotiq * pi/4 # If robotiq 2f-140 is on, go to pi/2, else if it is franka gripper, go to pi/4.
    ...
    def rotate(self, angle):
        ...
        joint_goal = group.get_current_joint_values()
        joint_goal[0] = joint_goal[0] + angle
        group.go(joint_goal, wait=True)
        group.stop()
        current_joints = self.group.get_current_joint_values()
        return self.all_joint_close(joint_goal, current_joints, 0.005)
    ...
    def home_routine(self):
        ...
        self.move_to_z(0.25)
        self.rotate(pi/2)
        self.move_by_xy(0, 0.15)
        self.go_to_joint_pose([1.5713446893481975, 0.05767666414223218, -0.001269726230882371, -2.5417385456533297, 0.001546808373802795, 2.601854643596543, 0.785518268720971])

        self.routine_unlock()

        print("Joint", self.group.get_current_joint_values())
        print("Pose", self.group.get_current_pose().pose)

    def pickup(self, x, y, appropriate=True):
    
        requests.get('http://emobotics-eye.glitch.me/change_emotion?emotion=idle')
        ...
        # robot going home
        self.move_to_z(0.16)
        self.grasp_gripper(0.01, 0.04, 1, True)
        self.move_to_z(0.25)
        self.rotate(-pi)
        self.move_by_xy(-x, -y)
        self.move_to_z(0.05)
        ...
        if appropriate: requests.get('http://emobotics-eye.glitch.me/change_emotion?emotion=happy')
        else: requests.get('http://emobotics-eye.glitch.me/change_emotion?emotion=sad')
    ...
    def fail(self, appropriate=True):
        ...
        self.move_to_z(0.25)
        self.rotate(-pi)
        self.move_by_xy(-0.3, 0)
        self.move_to_z(0.1)
        self.move_gripper(0.08, 0.04, True)

        if appropriate: requests.get('http://emobotics-eye.glitch.me/change_emotion?emotion=sad')
        else: requests.get('http://emobotics-eye.glitch.me/change_emotion?emotion=happy')
    ...
    def cmdCallback(self, data):
        ...
        #     self.controller_test()
            ...
            self.pickup(float(args[1]), float(args[2]))
        elif args[0] == 'fail':
            self.fail()
        elif args[0] == 'exp':
            if args[1] == '1': self.pickup(0, 0, appropriate)
            elif args[1] == '3': self.pickup(0, 0.2, appropriate)
            elif args[1] == '4': self.pickup(0.1, 0, appropriate)
            else: self.fail(appropriate)
