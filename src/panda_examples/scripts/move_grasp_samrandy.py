#!/usr/bin/env python
'''
This script was used for Sam/Randy's experiment. 
'''
import sys
import copy
import rospy
...
from robotiq_2f_gripper_control.msg import Robotiq2FGripper_robot_output as outputMsg #Comment for asus-pc
...
class MoveGrasp(object):
    def __init__(self):
        ...
        self.face_pub = rospy.Publisher('/face', String, queue_size = 1000) #publishing here saves only tactile data
        self.rbtq_pub = rospy.Publisher('Robotiq2FGripperRobotOutput', outputMsg, queue_size = 1000) #Comment for asus-pc
    ...
    def trajectory_saver(self, data):
        ...
        self.cartesian_plan_save_execute(0.2,[0.45,-0.1,0.07],self.home_quat,"obj_1_1.yaml")
        self.cartesian_plan_save_execute(0.2,[0.49,-0.1,0.07],self.home_quat,"obj_1_2.yaml")
        self.move_gripper(0.065)
        rospy.sleep(1)
        self.cartesian_plan_save_execute(0.2,[0.49,-0.1,0.09],self.home_quat,"obj_1_3.yaml")
        self.cartesian_plan_save_execute(0.2,[0.7,0,0.09],self.home_quat,"obj_1_4.yaml")
        self.cartesian_plan_save_execute(0.2,[0.7,0,0.07],self.home_quat,"obj_1_5.yaml")
        self.move_gripper(0.08)
        rospy.sleep(1)
        self.cartesian_plan_save_execute(0.2,[0.55,0,0.07],self.home_quat,"obj_1_6.yaml")
        rospy.sleep(1)
        self.go_to_home(robotiq = False)
        self.cartesian_plan_save_execute(0.2,[0.45,0.1,0.07],self.home_quat,"obj_2_1.yaml")
        self.cartesian_plan_save_execute(0.2,[0.49,0.1,0.07],self.home_quat,"obj_2_2.yaml")
        self.move_gripper(0.065)
        rospy.sleep(1)
        self.cartesian_plan_save_execute(0.2,[0.49,0.1,0.09],self.home_quat,"obj_2_3.yaml")
        self.cartesian_plan_save_execute(0.2,[0.75,0,0.09],self.home_quat,"obj_2_4.yaml")
        self.cartesian_plan_save_execute(0.2,[0.75,0,0.07],self.home_quat,"obj_2_5.yaml")
        self.move_gripper(0.08)
        rospy.sleep(1)
        self.cartesian_plan_save_execute(0.2,[0.55,0,0.07],self.home_quat,"obj_2_6.yaml")
        rospy.sleep(1)
        self.go_to_home(robotiq = False)
        self.routine_unlock()
    ...
    def pick_left(self):
        ...
        if not self.is_moveit_inited:
            self.move_it_init()
        self.go_to_home(robotiq = False)
        self.load_plan_execute("obj_1_1.yaml")
        self.load_plan_execute("obj_1_2.yaml")
        self.move_gripper(0.06)
        rospy.sleep(1)
        self.load_plan_execute("obj_1_3.yaml")
        self.load_plan_execute("obj_1_4.yaml")
        self.load_plan_execute("obj_1_5.yaml")
        self.move_gripper(0.08)
        rospy.sleep(1)
        self.load_plan_execute("obj_1_6.yaml")
        rospy.sleep(3)
        self.face_pub.publish("p2")
        rospy.sleep(3)
        self.go_to_home(robotiq = False)
        self.routine_unlock()
    ...
    def pick_right(self):
        ...
        if not self.is_moveit_inited:
            self.move_it_init()
        self.go_to_home(robotiq = False)
        self.load_plan_execute("obj_2_1.yaml")
        self.load_plan_execute("obj_2_2.yaml")
        self.move_gripper(0.06)
        rospy.sleep(1)
        self.load_plan_execute("obj_2_3.yaml")
        self.load_plan_execute("obj_2_4.yaml")
        self.load_plan_execute("obj_2_5.yaml")
        self.move_gripper(0.08)
        rospy.sleep(1)
        self.load_plan_execute("obj_2_6.yaml")
        rospy.sleep(3)
        self.face_pub.publish("p2")
        rospy.sleep(3)
        self.go_to_home(robotiq = False)
    ...
    def data_collection_routine(self, data):
        ...
        self.load_plan_execute('testing_cart.yaml')
        if not self.pose_check([0.55,0,0.415],[0.5**0.5, -(0.5**0.5),0,0]):
            self.something_wrong()
            return

        self.go_to_home(robotiq = True)

        self.load_plan_execute('testing_joint.yaml')
        if not self.joint_check([0,0.155528,0,-2.1929,0,2.34483,1.56957]):
            self.something_wrong()
            return
